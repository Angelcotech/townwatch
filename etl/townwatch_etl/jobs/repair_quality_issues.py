...
from ..repair.engine import run_repairs
...
class RepairQualityIssues(IngestJob):
    ...
    def ingest(self) -> None:
        ...
        print(f"\nrepair summary: {self.repair_summary}")
        self.rows_written = self.repair_summary["repaired"]

        if self.rerun_qa and not self.dry_run:
...
def main() -> int:
    ...
    result = job.run()
    print()
    print(json.dumps({"job": result, "repair": job.repair_summary}, indent=2, default=str))
    return 0
