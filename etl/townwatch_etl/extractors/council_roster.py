...
Output: list of CouncilMemberRecord with name, photo_url, email,
...
officials-table scraper doesn't capture (term end dates, direct emails).
Same content-type-dispatch shape as agendas/minutes/campaign_finance.
"""
...
class CouncilMemberRecord(BaseModel):
    ...
    email: str | None = Field(
        default=None,
        description="Direct email address if published on the page. Null if not shown.",
    )
...
For each member shown, extract:
  ...
  - direct email and phone, IF the page publishes them
...
2. Email: only populate if the page actually publishes it. Cities often
   omit direct email for privacy. Null is the correct value when absent.

3. Term dates: convert any human-readable date format to ISO YYYY-MM-DD.
...
4. District: only populate for district-based seats. For at-large seats
   leave it null.

5. Photo URL: if you see an <img> tag for a member, capture its src as
   an absolute URL (prepend the page's domain if needed). Null if no
   photo is shown.

6. Stale data signal: if the page header indicates a prior term (e.g.
   "2022 City Council" when reading in 2026), call this out in
   extraction_notes.

7. extraction_confidence: "high" if every field for every member was
